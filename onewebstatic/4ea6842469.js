window.OnewebContactForm = function ($) {
    ...
    ContactFormValidation.prototype = {
        ...
        getFormValidationErrors: function (formDOM) {
            var formFields = formDOM.find('.contact-form-field-container'), emailRegex = this.emailRegex, urlRegex = this.urlRegex, messageRegex = /^\S*/g, numberRegex = /^[0-9+\(\)#\.\s]+$/, errors = 0;
            $.each(formFields, $.proxy(function (index, element) {
                ...
                } else if ($(element).find('input[type="number"]')[0]) {
                    inputFieldVal = $(element).find('input[type="number"]').val();
                    if (errorMessage || inputFieldVal) {
                        errorMessage = errorMessage || decodeURIComponent(this.allFieldErrorMessage[index]);
        ...
        updateFormData: function (formDOM) {
            ...
            $.each(formFields, $.proxy(function (index, element) {
                ...
                if ($(element).find('input[type="text"]')[0] || $(element).find('input[type="url"]')[0] || $(element).find('input[type="number"]')[0] || $(element).find('input[type="tel"]')[0]) {
...
(function () {
    oneJQuery(function () {
        ...
        new window.OnewebContactForm({
            'formDOMId': '9FDBD337-0456-4D0C-88C4-D1D88E766F16',
